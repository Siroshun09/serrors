...
func TestWrapWith(t *testing.T) {
	...
	t.Run("nil error", func(t *testing.T) {
		got := serrors.WithMsg(nil, "test")
		if got == nil {
			t.Errorf("WithMsg = nil, want not nil")
		}

		if got.Error() != "test" {
			t.Errorf("WithMsg() = %v, want %v", got, "test")
		}
	})

	t.Run("without attrs", func(t *testing.T) {
		err := serrors.WithMsg(baseErr, "test")
		if err.Error() != "test\nbase error" {
			t.Errorf("WithMsg() = %v, want %v", err, "test\nbase error")
		}
		...
		if st[0].Function != "github.com/Siroshun09/serrors/v2_test.TestWrapWith.func2" {
			t.Errorf("stack trace does not contain current function: %v", st)
		}
	...
	t.Run("with attrs", func(t *testing.T) {
		err := serrors.WithMsg(baseErr, "test", attr1)
		if err.Error() != "test\nbase error" {
			t.Errorf("WithMsg() = %v, want %v", err, "test\nbase error")
		}
	...
	t.Run("wrap already has stack trace", func(t *testing.T) {
		...
		err2 := serrors.WithMsg(err1, "test")
		st2, _ := serrors.GetAttachedStackTrace(err2)
